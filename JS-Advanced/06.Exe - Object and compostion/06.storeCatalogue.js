...
function storeCatalogue(data) {
  ...
  //create object catalogue it letter: {name: price}
  let catalogue = {};
  ...
  for (let letter in catalogue) {
    ...
    Object.keys(catalogue[letter]) // sort keys(name) of every letter and print -> name: price;
      .sort((a, b) => a.localeCompare(b))
      .forEach((name) => {
        console.log(`  ${name}: ${catalogue[letter][name]}`);
      });
  }
}
