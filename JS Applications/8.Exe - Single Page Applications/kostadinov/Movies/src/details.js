...
async function deleteMovie(event, id){
  ...
  const confirmed = confirm('Are you sure want to delete this movie?');
  if(confirmed){

    
    const response = await fetch('http://localhost:3030/data/movies/' + id, {
      method:'delete',
      headers: {'X-Authorization': sessionStorage.getItem('auth-token')}
    });
...
function createMovieCard(movie, likes, ownLike) {
  ...
  if (userId != null) {
    if (userId == movie._ownerId) {
      controls.appendChild(
        e('a', { className: 'btn btn-danger', href: '#', onClick: (event) => deleteMovie(event, movie._id) },  'Delete')
      );
      controls.appendChild(
  ...
  controls.appendChild(spanLikes);
